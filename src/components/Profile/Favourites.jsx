...
const Favourites = () => {
  ...
  useEffect(() => {
    const fetchFavourites = async () => {
      try {
        const response = await axios.get(
          "https://bookstore-server-fuda.onrender.com/favourites/get-favourite", // Adjust endpoint for local development
          { headers }
        );
        setFavouriteBooks(response.data.data || []);
      } catch (error) {
  ...
  const handleToggleFavourite = async (bookId) => {
    try {
      const response = await axios.put(
        "https://bookstore-server-fuda.onrender.com/favourites/toggle-favourite", // This is the backend endpoint to toggle the favourite
