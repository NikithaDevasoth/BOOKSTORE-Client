...
const ViewBookDetails = () => {
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      ...
      try {
        const response = await axios.get(
          `https://bookstore-server-fuda.onrender.com/books/get-book-by-id/${id}`
        );
        setData(response.data.data);
      } catch (error) {
        console.error("Error fetching book details:", error);
  ...
  const handleFavourite = async () => {
    try {
      const response = await axios.put(
        "https://bookstore-server-fuda.onrender.com/favourites/add-favourite",
        { bookid: id },
        { headers }
      );
  ...
  const handleCart = async () => {
    try {
      const response = await axios.put(
        "https://bookstore-server-fuda.onrender.com/carts/add-to-cart",
        { bookid: id },
        { headers }
      );
  ...
  const deleteBook = async () => {
    try {
      const response = await axios.delete(
        `https://bookstore-server-fuda.onrender.com/books/delete-book/${id}`,
        { headers }
      );
      alert(response.data.message || "Book deleted successfully.");
      navigate("/all-books");
